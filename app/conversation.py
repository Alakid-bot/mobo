...
class ConversationWindow:
    """Bounded local record of users who recently had direct interaction."""
    ...
    def __init__(
        ...
        *,
        ...
        self._entries: OrderedDict[ConversationKey, float] = OrderedDict()
    ...
    def record(
        ...
        *,
        ...
        if at is not None and timestamp is not None:
            raise TypeError("use either at or timestamp, not both")
        event_time = at if at is not None else timestamp
        now = self._clock() if event_time is None else float(event_time)
        self._purge(now)
        if self.ttl_seconds <= 0:
            return
    ...
    def is_continuous(
        ...
        *,
        ...
        if timestamp is None:
            return False
        self._entries.move_to_end(key)
        return current - timestamp < self.ttl_seconds
    ...
    # Names useful to integrations that prefer event-oriented vocabulary.
    touch = record
    record_interaction = record
    has_recent = is_continuous
    is_recent = is_continuous
    should_continue = is_continuous
    ...
    def forget(
        ...
        self._entries.pop(_coerce_key(guild_id, channel_id, user_id), None)
