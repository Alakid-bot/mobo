...
2. **模型中心**：填写提供方所需的 API Key 或服务地址，拉取模型列表，先用固定无隐私提示真实测试，再启用通过测试的模型。
3. **全部配置**：确认公网地址、时区、核心人格与成本边界；模型角色在此只读，所有启用操作统一在模型中心完成。
4. **记忆与保留期**：确认原始消息、自动记忆、安全事件、反馈事件和运行统计的保留天数。
5. **安全与权限 → 安全规则**：填写规范提示词、违禁词和需要脱敏的输出规则。
6. **性格与行为 → 频道授权**：只打开确实允许机器人读取上下文的频道。
7. 如果要主动发言，先为频道打开“监听 + 主动”，再到全部配置打开全局总开关。建议观察一段时间后逐步提高概率和日上限。
8. 如需更换首次密码，在安全页修改；修改会注销全部管理会话。

## 7. Discord 验收

用普通成员账号检查：

- 可以看到 `/帮助`、`/状态`、`/记住`、`/我的记忆`、`/忘记我`、`/隐私`、`/关系`、`/喜好`。
- 不应看到管理员命令。
...
| 拉取模型失败 | API Key、提供方 Base URL、网络连通性；Ollama/兼容接口必须能从 Zeabur 容器访问 |
