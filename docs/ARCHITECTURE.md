...
新的直接触发消息会取消同一会话键的旧任务；没有提及或回复 mobo 的普通消息不会获得回复权，也不会中断正在生成的直接回复。即使模型 SDK 没有及时响应取消，发送前后的 generation version 检查也阻止旧结果输出。`/忘记我` 还会阻止新接入、递增版本、等待已接纳任务、取消动态总结并清空短期消息缓冲，避免删除后旧任务复活数据。
